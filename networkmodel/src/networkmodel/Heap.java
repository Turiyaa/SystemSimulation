...
public class Heap<Event extends Comparable<Event>>
{
   private static final int CAPACITY = 2;

   private int size;            // Number of elements in heap
   private Event[] heap;     // The heap array

   public Heap()
   {
      size = 0;
      heap = (Event[]) new Comparable[CAPACITY];
   }
  ...
  */
   ...
   {
      size = array.length;
      heap = (Event[]) new Comparable[array.length+1];
  ...
  */
   public void heapSort(Event[] array)
   {
      size = array.length;
      heap = (Event[]) new Comparable[size+1];
      System.arraycopy(array, 0, heap, 1, size);
      buildHeap();

      for (int i = size; i > 0; i--)
      {
         Event tmp = heap[i]; //move top item to the end of the heap array
         heap[i] = heap[1];
         heap[1] = tmp;
         size--;
         percolatingDown(1);
  ...
  */
   public Event deleteMin() throws RuntimeException
   {
  ...
  */
   ...
   {
      Event [] old = heap;
      heap = (Event []) new Comparable[heap.length * 2];
